...
import com.kodlib.megacalculator.utils.CalculateFactorial;
...
public class ScientificCal extends AppCompatActivity {
    ...
    private TextView mode, toggle;
    private TextView square, xpowy, log, sin, cos, tan, sqrt, fact;
...
//
    ...
    public void onClick(View v) {
        ...
        if (id == R.id.backButton) {
            super.onBackPressed();
        } else if (id == R.id.toggle) {//change the button text if switch button is clicked
            if (toggleMode == 1) {
                toggle.setTag(2);
        ...
        } else if (id == R.id.mode) {//change the angle property for trignometric operations if mode button is clicked
            if (angleMode == 1) {
                mode.setTag(2);
